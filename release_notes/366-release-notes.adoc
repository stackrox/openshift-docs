...
{product-title} ({product-title-short}) 3.66 includes feature enhancements, bug fixes, scale improvements, and other changes.
...
* You can now configure tolerations for Central, Scanner, ScannerDB, Sensor, and Admission Controller in {product-title} by using Helm charts and the {product-title-short} Operator.

* You can now disable the automatic administrator password generation for Central by specifying the `adminPasswordGenerationDisabled` as `true` in the {product-title-short} Operator configuration.
...
* The admission controller settings for the {product-title-short} Operator now listen to both `update` and `create` events by default.
