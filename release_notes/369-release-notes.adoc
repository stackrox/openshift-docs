...
With {product-title} 3.69, a lightweight version of scanner is delivered as part of secured cluster services on OpenShift by default to more effectively scan OpenShift Container Registry. For non-OpenShift environments this scanner is optional. OpenShift users not using the {product-title} are advised to update their helm charts to take advantage of these new capabilities.
...
[id="resolved-in-version-3691"]
=== Resolved in version 3.69.1

Release date: March 28, 2022

* *ROX-9833*: Previously, the {product-title-short}, removing enforcement from a built-in policy would result in an error. This issue has been fixed.
* *ROX-9834*: Previously, when used in policy criteria, the greater than and less than operators did not work correctly. This is has been fixed.
