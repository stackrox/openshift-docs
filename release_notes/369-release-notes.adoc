...
=== Enhanced support for OpenShift Container Registry (OCR)
With {product-title} 3.69, a lightweight version of scanner is delivered as part of secured cluster services on OpenShift by default to more effectively scan OpenShift Container Registry. For non-OpenShift environments this scanner is optional. OpenShift users not using the {product-title} are advised to update their helm charts to take advantage of these new capabilites.
...
* *ROX-9587*: Previously, e-mailed vulnerability reports were incompatible with some e-mail clients. This issue has been fixed.
...
{product-title} 3.69 includes new fields for vulnerabilities contained within an image which you use to sort and filter the vulnerabilities list.

[id="uefi-ebpf"]
=== Enhanced compatibility with UEFI secure boot
Because Collector is incompatible with UEFI secure boot when collecting runtime data using kernel modules. In {product-title} 3.69, when Collector detects that the host is using UEFI secure boot, it automatically fails over to use EBPF probes to prevent service disruption.
...
== Deprecation Notice
Red Hat is deprecating the following features in {product-title} 3.69. 

Red Hat will remove the following features in {product-title} 3.71.0:
...
Red Hat will remove these features in {product-title} 3.70.0:

* *Default Policy Deletion* will no longer be supported. Please disable default policies rather than deleting them going forward.
* *PolicyFields* in `/v1/policies` will no longer be exposed in API responses. This field is not modifiable in the REST API.
For any questions, please contact Red Hat Support at support@redhat.com.
