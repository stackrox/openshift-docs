...
{product-title} 3.69 includes a lightweight version of Scanner as part of the secured cluster services on {ocp} by default to more effectively scan the OpenShift Container Registry.
For non-OpenShift environments, this Scanner is optional.
...
* In {product-title-short} 3.70, Red Hat will remove the support for security policies that do not have a `policyVersion`. Therefore, if you have some externally stored older policies, you must convert them to use `policyVersion` 1.1. To do this, import the old policies into {product-title-short} and then export them again. You can check the `policyVersion` field for your stored policies to identify if they need conversion.
