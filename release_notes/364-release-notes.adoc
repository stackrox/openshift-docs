...
{product-title} ({product-title-short}) 3.64 includes feature enhancements, bug fixes, scale improvements, and other changes.
...
* *ROX-7850*: Due to a bug in the previous {product-title-short} Operator image, configuring the proxy support in the Operator Lifecycle Manager would incorrectly send internal traffic through the proxy.
The bug caused internal communication failure, and the {product-title-short} services would fail to start.
The updated image uses the fully qualified domain names for {product-title-short} services to fix this issue.
* *ROX-7872*: The updated Operator image sets the memory limit to 1 GiB and memory requests to 200 MiB to address out-of-memory issues when using the {product-title-short} Operator at scale.
