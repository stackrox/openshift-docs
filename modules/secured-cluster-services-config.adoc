...
| `scanner.disable`
| If you set this option to `true`, {product-title} deploys a lightweight scanner and scannerdb in the secured cluster to allow scanning images on OpenShift Container Registry. It's reccomended this be set to true on OpenShift or clusters relying on a registry not exposed outside of the cluster. Defaults to `false`
